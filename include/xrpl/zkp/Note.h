...
{
    std::uint64_t amount;
    uint256 rho;   // uniqueness randomizer
    uint256 r;     // commitment randomness
    uint256 a_pk;  // recipient/public key material

    ZkNote() = default;

    ZkNote(
        std::uint64_t const& amt,
        uint256 const& rho_,
        uint256 const& r_,
        uint256 const& key_)
...
// append a 64-bit integer in big-endian order
inline void
...
//XRPL hash = SHA-512 then first 32 bytes
inline uint256
...
// commitment = sha512Half( 0x01 || amount(8) || rho(32) || r(32) || a_pk(32) )
inline uint256
...
// Deposit witness:
// mode(1) || amount(8) || rho(32) || r(32) || a_pk(32)
// total = 105 bytes
inline Blob
packWitnessDeposit(ZkNote const& note)
{
    Blob out;
    out.reserve(1 + 8 + 32 + 32 + 32);

    out.push_back(kModeDeposit);
...
// Spend witness (using old note data)
// mode(1) || amount(8) || rho(32) || r(32) || a_pk(32) || a_sk(32)
// total = 137 bytes
inline Blob
packWitnessSpend(ZkNote const& note, uint256 const& a_sk)
{
    Blob out;
    out.reserve(1 + 8 + 32 + 32 + 32 + 32);

    out.push_back(kModeSpend);
    append_u64_be(out, note.amount);
    append_u256(out, note.rho);
    append_u256(out, note.r);
    append_u256(out, note.a_pk);
